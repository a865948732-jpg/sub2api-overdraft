...
func TestV020PoliciesComposeWithCustomPromptAndOverdraft(t *testing.T) {
	t.Cleanup(func() { SetCodexQuotaOverdraftEnabled(false) })
	SetCodexQuotaOverdraftEnabled(true)

	groupID := int64(5)
	group := &Group{ID: groupID, Platform: PlatformOpenAI, Status: StatusActive, Hydrated: true, ForceOpenAIFast: true, FreeOpenAIFast: true, MaxReasoningEffort: "medium"}
